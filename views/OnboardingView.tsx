...
import { Aperture, ArrowRight, Camera, Briefcase, Building, Zap, Check, Loader2, User as UserIcon, Clock, DollarSign, Tag, ArrowLeft, MapPin, Phone, Plus, Trash2, Percent, Globe } from 'lucide-react';
...
const OnboardingView: React.FC<OnboardingViewProps> = ({ user, onComplete }) => {
  ...
  useEffect(() => {
      if(step === 1 && user.name && !studioName) {
          const name = `${user.name.split(' ')[0]}'s Studio`;
          setStudioName(name);
      }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-lumina-base flex items-center justify-center relative overflow-hidden p-6">
      ...
      >
        ...
        <div className="bg-lumina-surface/90 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl min-h-[500px] flex flex-col justify-center relative overflow-hidden">
            
            {/* Progress Bar */}
            {step < 6 && (
                <div className="absolute top-0 left-0 w-full h-1 bg-lumina-base">
                    <div className="h-full bg-lumina-accent transition-all duration-500" style={{ width: `${(step/5)*100}%` }}></div>
                </div>
            )}
            ...
            <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                    ...
                    >
                        <div className="text-center mb-8">
                            <h2 className="text-xl font-bold text-white">Studio Brand Identity</h2>
                            <p className="text-lumina-muted text-sm">Configure your invoices and public website URL.</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="text-xs font-bold text-lumina-muted uppercase mb-1 block">Public Site URL</label>
                                <div className="relative">
                                    <Globe size={16} className="absolute left-3 top-3 text-lumina-muted"/>
                                    <input 
                                        type="text" 
                                        value={subdomain}
                                        onChange={(e) => setSubdomain(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                                        className="w-full bg-lumina-base border border-lumina-highlight rounded-xl p-3 pl-10 pr-24 text-white focus:border-lumina-accent outline-none text-sm font-mono"
                                        placeholder="studio-name"
                                    />
                                    <span className="absolute right-4 top-3 text-lumina-muted text-sm pointer-events-none">.lumina.id</span>
                                </div>
                            </div>
                            <div>
                        ...
                        <div className="flex justify-center mt-8">
                            <button 
                                onClick={() => { if(studioName && subdomain && address && phone) setStep(2); }}
                                disabled={!studioName || !subdomain || !address || !phone}
                ...
                {step === 3 && (
                    ...
                    >
                        <div className="text-center mb-6">
                            <h2 className="text-xl font-bold text-white">Operational Logistics</h2>
                            <p className="text-lumina-muted text-sm">Define your working hours and available spaces.</p>
                        </div>
                ...
                {step === 5 && (
                    ...
                    >
                        <div className="text-center mb-6">
                            <h2 className="text-xl font-bold text-white">Create Primary Service</h2>
                            <p className="text-lumina-muted text-sm">Add your main package. You can add more later.</p>
                        </div>
                        ...
                        <div className="bg-lumina-base border border-lumina-highlight rounded-2xl p-6 max-w-md mx-auto">
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <span className="font-bold text-white uppercase tracking-wider text-sm">Primary Package</span>
                            </div>
                        ...
                        <div className="flex justify-center mt-8 gap-4">
                            ...
                            <button 
                                onClick={() => { if(pkg.name && pkg.price > 0) setStep(6); }}
