...
import { Settings as SettingsIcon, Tag, Plus, Edit2, Building, Save, Trash2, HardDrive, RefreshCcw, Archive, User as UserIcon, Zap, Loader2 } from 'lucide-react';
...
const SettingsView: React.FC<ExtendedSettingsViewProps> = ({ packages, config, onAddPackage, onUpdatePackage, onDeletePackage, onUpdateConfig, bookings = [], currentUser, onUpdateUserProfile, onDeleteAccount, googleToken, setGoogleToken, assets = [], users = [] }) => {
  ...
  // ... Google Logic ...
  const loadGoogleScript = () => { /* ... */ return Promise.resolve(true); };
  const handleConnectGoogle = async () => { /* ... */ };

  // --- SAFE HANDLERS WITH INTEGRITY CHECK ---
  ...
  const handleArchivePackage = (e: React.MouseEvent, pkg: Package) => {
      e.stopPropagation();
      // Integrity Check: Warn if future bookings exist
      ...
      if (futureBookings.length > 0) {
          confirmMsg += `\n\nWARNING: There are ${futureBookings.length} upcoming bookings using this package. Archiving it will hide it from new bookings but keep existing ones.`;
      }
  ...
  // ... (Other handlers like handleAddRoom, handleSaveProfile, handleExportData etc. keep essentially same) ...
      ...
      <div className="flex-1 bg-lumina-surface border border-lumina-highlight rounded-2xl p-6 lg:p-8 relative overflow-y-auto custom-scrollbar">
        ...
        {activeTab === 'GENERAL' && (
            <div className="space-y-8">
                ...
                {/* Rooms Management with Delete Protection */}
                <div className="space-y-4">
                ...
                <div className="flex justify-end pt-6 sticky bottom-0 bg-lumina-surface py-4 border-t border-lumina-highlight">
                    <button 
                        ...
                        className="bg-lumina-accent text-lumina-base px-6 py-3 rounded-xl font-bold hover:bg-lumina-accent/90 shadow-lg w-full md:w-auto flex items-center justify-center gap-2"
                    >
                    ...
                    <div className="bg-lumina-base border border-lumina-highlight rounded-xl p-6 space-y-4 mb-6 animate-in slide-in-from-top-4">
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            ...
                            <button onClick={handleSavePackage} disabled={isSavingPackage} className="bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2">
                                {isSavingPackage && <Loader2 className="animate-spin" size={16} />}
                                Save Package
                            </button>
                        </div>
                    </div>
        ...
        {/* ... (Automation and Data tabs remain mostly the same, ensuring integrity in delete handlers) ... */}
        ...
        {activeTab === 'DATA' && (
            <div className="space-y-6">
                <div className="p-6 bg-lumina-base border border-lumina-highlight rounded-xl text-center">
                    ...
                    <p className="text-sm text-lumina-muted mb-6">Download a full JSON backup.</p>
