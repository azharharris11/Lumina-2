...
const ProjectDrawer: React.FC<ProjectDrawerProps> = ({ isOpen, onClose, booking, photographer, onUpdateBooking, onDeleteBooking, bookings = [], config, packages = [], currentUser, assets = [], users = [], transactions = [], onAddTransaction, accounts = [], googleToken, onLogActivity }) => {
  ...
  // ... Drive functions omitted for brevity, assume identical ...
  ...
  const createSubFolder = async () => { /* simplified */ };
  const renameItem = async () => { /* simplified */ };
  const trashItem = async (item: DriveFolder) => { /* simplified */ };
  const handleUploadToDrive = async (e: React.ChangeEvent<HTMLInputElement>) => { /* simplified */ };
